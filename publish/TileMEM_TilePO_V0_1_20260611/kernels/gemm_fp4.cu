...
__device__ __forceinline__ int sign_extend_4(unsigned int value) {
  return (value & 0x8U) ? static_cast<int>(value | ~0xFU) : static_cast<int>(value);
}

__device__ __forceinline__ float decode_fp4_sample(const unsigned char *packed, const float *scale, int index, int col) {
  unsigned char byte = packed[index >> 1];
  unsigned int raw = (index & 1) ? (static_cast<unsigned int>(byte) >> 4) : (static_cast<unsigned int>(byte) & 0xFU);
  return static_cast<float>(sign_extend_4(raw)) * scale[col / kScaleBlockN];
}

extern "C" __global__ void tilemem_gemm_fp4_kernel(
    const float *a,
  ...
  for (int kk = 0; kk < k; ++kk) {
    int index = kk * n + col;
    float av = a[row * k + kk];
    float bv = decode_fp4_sample(b_packed, scale, index, col);
    acc += av * bv;
  }
  c[row * n + col] = acc;
}
...
std::vector<unsigned char> make_b(int k, int n) {
  int elements = k * n;
  std::vector<unsigned char> values(static_cast<size_t>((elements + 1) / 2), 0);
  for (int i = 0; i < elements; ++i) {
    int q = ((i * 5 + 1) % 15) - 7;
    unsigned int raw = static_cast<unsigned int>(q) & 0xFU;
    unsigned char encoded = static_cast<unsigned char>(raw);
    if (i & 1) {
      values[static_cast<size_t>(i >> 1)] |= static_cast<unsigned char>(encoded << 4);
    } else {
      values[static_cast<size_t>(i >> 1)] |= encoded;
    }
  }
  return values;
}
...
std::vector<float> make_scales(int n) {
  ...
  for (int i = 0; i < blocks; ++i) {
    values[static_cast<size_t>(i)] = 0.0625f * (1.0f + static_cast<float>(i % 5) * 0.125f);
  }
  return values;
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 3; ++i) {
    tilemem_gemm_fp4_kernel<<<blocks, threads>>>(d_a, d_b, d_scale, d_c, m, n, k);
  }
  ...
  std::printf(
      "{\"precision\":\"fp4\",\"status\":\"success\",\"m\":%d,\"n\":%d,\"k\":%d,"
...
extern "C" cudaError_t tilemem_launch_gemm_fp4(
    const float *a,
    const unsigned char *b_packed,
    const float *scale,
    float *c,
  ...
  dim3 threads(kThreadsX, kThreadsY);
  dim3 blocks((n + threads.x - 1) / threads.x, (m + threads.y - 1) / threads.y);
  tilemem_gemm_fp4_kernel<<<blocks, threads, 0, stream>>>(a, b_packed, scale, c, m, n, k);
