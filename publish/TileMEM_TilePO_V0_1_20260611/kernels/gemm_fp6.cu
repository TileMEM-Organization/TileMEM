...
constexpr int kScaleBlockN = 64;

__device__ __forceinline__ int sign_extend_6(unsigned int value) {
  return (value & 0x20U) ? static_cast<int>(value | ~0x3FU) : static_cast<int>(value);
}

__device__ __forceinline__ float decode_fp6_sample(const unsigned char *packed, const float *scale, int index, int col) {
  int bit_offset = index * 6;
  int byte_offset = bit_offset >> 3;
  int shift = bit_offset & 7;
  unsigned int window = static_cast<unsigned int>(packed[byte_offset]) |
                        (static_cast<unsigned int>(packed[byte_offset + 1]) << 8);
  unsigned int raw = (window >> shift) & 0x3FU;
  return static_cast<float>(sign_extend_6(raw)) * scale[col / kScaleBlockN];
}

extern "C" __global__ void tilemem_gemm_fp6_kernel(
    const float *a,
  ...
  for (int kk = 0; kk < k; ++kk) {
    int index = kk * n + col;
    float av = a[row * k + kk];
    float bv = decode_fp6_sample(b_packed, scale, index, col);
    acc += av * bv;
  }
  c[row * n + col] = acc;
}
...
void set_packed_bit(std::vector<unsigned char> &buffer, int bit_index, bool value) {
  if (value) {
    buffer[static_cast<size_t>(bit_index >> 3)] |= static_cast<unsigned char>(1U << (bit_index & 7));
  }
}

std::vector<unsigned char> make_b(int k, int n) {
  int elements = k * n;
  std::vector<unsigned char> values(static_cast<size_t>((elements * 6 + 7) / 8 + 1), 0);
  for (int i = 0; i < elements; ++i) {
    int q = ((i * 11 + 7) % 63) - 31;
    unsigned int raw = static_cast<unsigned int>(q) & 0x3FU;
    int base_bit = i * 6;
    for (int bit = 0; bit < 6; ++bit) {
      set_packed_bit(values, base_bit + bit, ((raw >> bit) & 1U) != 0);
    }
  }
  return values;
}
...
std::vector<float> make_scales(int n) {
  ...
  for (int i = 0; i < blocks; ++i) {
    values[static_cast<size_t>(i)] = 0.015625f * (1.0f + static_cast<float>(i % 5) * 0.125f);
  }
  return values;
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 3; ++i) {
    tilemem_gemm_fp6_kernel<<<blocks, threads>>>(d_a, d_b, d_scale, d_c, m, n, k);
  }
  ...
  std::printf(
      "{\"precision\":\"fp6\",\"status\":\"success\",\"m\":%d,\"n\":%d,\"k\":%d,"
...
extern "C" cudaError_t tilemem_launch_gemm_fp6(
    const float *a,
    const unsigned char *b_packed,
    const float *scale,
    float *c,
  ...
  dim3 threads(kThreadsX, kThreadsY);
  dim3 blocks((n + threads.x - 1) / threads.x, (m + threads.y - 1) / threads.y);
  tilemem_gemm_fp6_kernel<<<blocks, threads, 0, stream>>>(a, b_packed, scale, c, m, n, k);
